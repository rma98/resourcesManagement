describe("Cadastro de Professor", () => {
    beforeEach(() => {
        ...
        // Clica no link para a página de cadastro de professor
        cy.get("a[href='/register-professor']").click();
    });

    it("Deve preencher o formulário e cadastrar um professor com sucesso", () => {
        cy.get("input[placeholder='Digite seu nome completo']").type("Leonardo Lima");
        cy.get("input[placeholder='Digite seu email']").type("leonardo.lima@example.com");
    ...
    it("Deve impedir cadastro caso o nome tenha menos de duas palavras", () => {
        cy.get("input[placeholder='Digite seu nome completo']").type("Leonardo");
        cy.get("input[placeholder='Digite seu email']").type("leonardo.lima@example.com");
        cy.get("input[placeholder='Digite sua senha']").type("SenhaForte@123");
    ...
    it("Deve exibir erro ao tentar cadastrar com um email já cadastrado", () => {
        cy.get("input[placeholder='Digite seu nome completo']").type("Leonardo Lima");
        cy.get("input[placeholder='Digite seu email']").type("leonardo.lima@example.com"); // Email já existente
        cy.get("input[placeholder='Digite sua senha']").type("SenhaForte@123");
    ...
    it("Deve exibir erro ao tentar cadastrar com um nome muito longo", () => {
        cy.get("input[placeholder='Digite seu nome completo']").type("Leonardo".repeat(20));
        cy.get("input[placeholder='Digite seu email']").type("leonardo.lima@example.com");
        cy.get("input[placeholder='Digite sua senha']").type("SenhaForte@123");
